...
from imagekit.processors import ResizeToFill, Transpose, SmartResize,ResizeToFit,Thumbnail
...
def get_upload_path(instance, filename):
        ...
        ext = ext.lower()
        file_path = '{username}/{trip_slug}/{experience}/{name}{ext}'.format(
            username=instance.experience.trip.user.username,trip_slug=instance.experience.trip.slug,experience=instance.experience_id, name=name, ext=ext) 
        return file_path

def get_upload_path_big(instance, filename):
        import os
        name = os.path.splitext(filename)[0]
        ext = os.path.splitext(filename)[1]
        ext = ext.lower()
        file_path = '{username}/{trip_slug}/{experience}/{name}_big{ext}'.format(
            username=instance.experience.trip.user.username,trip_slug=instance.experience.trip.slug,experience=instance.experience_id, name=name, ext=ext) 
        return file_path
...
def get_upload_path_small(instance, filename):
        ...
        ext = os.path.splitext(filename)[1]
        ext = ext.lower()
        file_path = '{username}/{trip_slug}/{experience}/{name}_small{ext}'.format(
            username=instance.experience.trip.user.username,trip_slug=instance.experience.trip.slug,experience=instance.experience_id, name=name, ext=ext) 
        return file_path
...
('ZW',  '-19.015438, 29.154857')
    ...
    lat_lng = models.CharField(max_length=100, blank = True, choices=LAT_LNG_CHOICES)
...
class Picture(models.Model):
    experience = models.ForeignKey(Experience, related_name='image_set')
    picture = models.ImageField(upload_to=get_upload_path, blank=True)
    picture_big = ProcessedImageField(upload_to=get_upload_path_big,
                                        blank=True,
                                        processors=[Transpose(),Thumbnail(600)],
                                        format='JPEG',
                                        options={'quality': 100})
    picture_small = ProcessedImageField(upload_to=get_upload_path_small,
                                           blank=True,
                                           processors=[Transpose(),Thumbnail(300)],
                                           format='JPEG',
                                           options={'quality': 100})
    
    def clearImageCache(self):
       "invalidate all ImageKit spec caches"
       self.picture_big.generate()
       self.picture_small.generate()
    #def __str__(self):              # __unicode__ on Python 2
        #return self.picture